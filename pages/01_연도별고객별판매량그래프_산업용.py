...
def load_summary_data():
    # 데이터 경로 설정 (상위 폴더 구조에 맞춰 data/ 폴더 참조)
...
if df_raw is not None:
    ...
    # 기준 연도별 타겟 리스트 (선택한 순위 범위의 업체 추출)
    ...
    st.info(f"💡 **{base_year}년** 실적 기준 TOP {start_rank}~{end_rank} 분석 (단위: {unit_label})")

    if not df_plot.empty:
        # 1. 클러스터 막대 그래프 (기존의 꺾은선 제거 후 최상단 배치)
        ...
        # 3. 상세 실적 테이블
        st.subheader(f"📊 {base_year}년 기준 주요 고객 상세 실적")
        ...
        st.warning("분석할 데이터가 없습니다. 사이드바에서 범위를 조절해 주세요.")

else:
    st.error("데이터 파일(industry_yearly_summary.csv)을 찾을 수 없습니다. 경로를 확인해 주세요.")